	...
	<div id="detailarea">
		...
		<div id="detailrightblock">
			...
			<div>
                ...
                <!--{* ▼▼▼↓定期購入設定中の商品で「カゴに入れる」ボタンを非表示にする場合はif文を有効にする ▼▼▼*}-->
				<!--{*if $arrProduct.regular != 1*}-->
					...
					<span class="price1">
						<!--{if $arrProduct.price02_min == $arrProduct.price02_max}-->
							<!--{$arrProduct.price02_min|reduce_price:$arrProduct.product_id:$session_customer_id|sfPreTax:$arrSiteInfo.tax:$arrSiteInfo.tax_rule|number_format}-->
						<!--{else}-->
							<!--{$arrProduct.price02_min|reduce_price:$arrProduct.product_id:$session_customer_id|sfPreTax:$arrSiteInfo.tax:$arrSiteInfo.tax_rule|number_format}-->～<!--{$arrProduct.price02_max|reduce_price:$arrProduct.product_id:$session_customer_id|sfPreTax:$arrSiteInfo.tax:$arrSiteInfo.tax_rule|number_format}-->
						<!--{/if}-->円
					</span>
					<p>
						<!--{if $arrProduct.price01_max > 0}-->
							...
							<span>
								<!--{if $arrProduct.price01_min == $arrProduct.price01_max}-->
									<!--{$arrProduct.price01_min|sfPreTax:$arrSiteInfo.tax:$arrSiteInfo.tax_rule|number_format}-->
								<!--{else}-->
									<!--{$arrProduct.price01_min|sfPreTax:$arrSiteInfo.tax:$arrSiteInfo.tax_rule|number_format}-->～<!--{$arrProduct.price01_max|sfPreTax:$arrSiteInfo.tax:$arrSiteInfo.tax_rule|number_format}-->
								<!--{/if}-->円
							</span>
						<!--{/if}-->
					</p>
					...
					<!--{*if $arrProduct.regular != 1*}-->
						...
						<!--{if $tpl_stock_find}-->
							...
							<!--★▼カートに入れる▼★-->
								<ul>
									<li class="sp_btn">
										<a href="javascript:void();" onclick="document.form1.submit();">カートに入れる<input type="hidden" name="cart" id="cart" /></a>
									</li>
								</ul>
							<!--★▲カートに入れる▲★-->
						<!--{else}-->
					...
					<!--{if $arrProduct.regular != 0}-->
						<div class="teiki">
							...
							<form name="form2" id="form2" method="post" action="<!--{$smarty.server.REQUEST_URI|omit_query_part|escape}-->">
								...
								<!--★▼カートに入れる▼★-->
									<div class="sp_btn">
										<a href="javascript:void();" onclick="document.form2.submit();" >カートに入れる<input type="hidden" name="cart2" id="cart2" /></a>
									</div>
								<!--★▲カートに入れる▲★-->
								<!--{else}-->
								...
								<a href="/smp<!--{$smarty.const.DETAIL_P_HTML}--><!--{$arrRecommend[cnt].product_id}-->">
									...
									<p>
										...
										<!--{if $price02_min == $price02_max}-->
											<!--{$price02_min|reduce_price:$arrRecommend[cnt].product_id:$session_customer_id|sfPreTax:$arrSiteInfo.tax:$arrSiteInfo.tax_rule|number_format}-->
										<!--{else}-->
											<br />
											<!--{$price02_min|reduce_price:$arrRecommend[cnt].product_id:$session_customer_id|sfPreTax:$arrSiteInfo.tax:$arrSiteInfo.tax_rule|number_format}-->円&nbsp;～&nbsp;
											<!--{$price02_max|reduce_price:$arrRecommend[cnt].product_id:$session_customer_id|sfPreTax:$arrSiteInfo.tax:$arrSiteInfo.tax_rule|number_format}-->
										<!--{/if}-->円</span>
									</p>
