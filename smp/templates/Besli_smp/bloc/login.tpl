...
<div id="loginarea">
	...
	<form name="login_form" id="login_form" method="post" action="<!--{$smarty.const.SSL_URL}-->smp/frontparts/login_check.php" onsubmit="return fnCheckLogin('login_form')">
		...
		<div id="login">
			...
			<!--{else}-->
				<p>
				<span class="align_left">メールアドレス</span>
				<input type="text" autocorrect="off" autocapitalize="off" name="login_email" class="box96" value="<!--{$tpl_login_email|escape}-->" />
				</p>
				<p>
				<span class="align_left">パスワード</span>
				<input type="password" autocorrect="off" autocapitalize="off" name="login_pass" class="box96" />
				</p>
		</div>
				...
				<ul>
					<li class="sp_btn">
						<a href="javascript:void();" onclick="document.login_form.submit();">ログイン<input type="hidden" name="subm" /></a>
					</li>
					<li class="nm_btn">
