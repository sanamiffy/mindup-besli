...
<div class="payment_info">
    ...
    <select name="payment_id" id="payment_select" onchange="selectPaymentMethod();">
        ...
        <option value="<!--{$item.payment_id}-->" <!--{if $item.payment_id == $arrForm.payment_id.value}-->selected="selected"<!--{/if}-->><!--{$item.payment_method}--></option>
        <!--{/foreach}-->
    </select>
    </div>
...
<!--{/if}-->
	...
	<ul>
		<li class="mg_cardbtn3">
			<span class="btn_next"><a href="javascript:void();" onclick="paygent_token_send();">支払方法を変更</a></span>
		</li>
	</ul>
...
$(function(){
	...
	// $('select[name=payment_id]').click(function() {
	...
	// if ($('select[name=payment_id]:selected').hasClass('credit_check')) {
	// 	$('input[name=credit]').val('1');
	// 	$('.credit_info').show();
	// 	if ($('select[name=payment_id]:selected').hasClass('gmo_credit')) {
	// 		$('.gmo_method').show();
	// 		$('.credit_paygent').hide();
	// 	} else {
	// 		<!--{if $arrSavedCardInfo.regular}-->$('.credit_info').hide();<!--{/if}-->
