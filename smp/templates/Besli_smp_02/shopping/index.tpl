<div id="shoppingcolumn">
	<div class="shopping_inner">
		...
		<ul>
			<li class="sp_btn">
				...
				<!--{else}-->
					...
					<!--{else}-->
						<a href="/smp/shopping/deliv_smp.php?sp=<!--{$session}-->">会員登録</a>
					<!--{/if}-->
				<!--{/if}-->
			</li>
		</ul>
		...
		<div style="padding-top:10px;">
			...
			<ul>
				<li class="sp_btn">
					<span class="btn_next">
						<a href="javascript:void();" onclick="document.member_form2.submit();">購入手続きへ<input type="hidden" name="buystep" id="buystep" /></a>
					</span>
				</li>
			</ul>
			</form>
		...
		<!--{/if}-->
			<div class="in_loginarea">
				...
				<div class="inputbox">
					...
					<p>
						<img src="<!--{$TPL_DIR}-->img/side/icon_mail.gif" width="40" height="21" alt="メールアドレス"  align="middle" /><input class="input_tx" type="text" name="<!--{$key}-->" value="<!--{$tpl_login_email|escape}-->" maxlength="<!--{$arrForm[$key].length}-->" style="width:220px;<!--{$arrErr[$key]|sfGetErrorColor}-->; ime-mode: disabled;" />
					</p>
					<p class="passwd">
	...
	<ul>
		<li class="sp_btn">
			<a href="javascript:void();" onclick="document.member_form.submit();">ログイン<input type="hidden" name="log" id="log"/></a>
		</li>
	</ul>
