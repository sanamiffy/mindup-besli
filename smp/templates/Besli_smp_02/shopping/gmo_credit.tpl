	...
	<div class="shopping_inner">
		...
		<form name="form1" id="form1" method="post" action="<!--{$smarty.server.PHP_SELF|escape}-->">
			...
			<table summary="決済情報の入力">
				...
				<tr>
					<td>
						...
						<input class="inout_tx" type="text" name="<!--{$key1}-->" value="<!--{$arrForm[$key1].value|escape}-->" maxlength="<!--{$arrForm[$key1].length}-->" style="ime-mode: disabled; <!--{$arrErr[$key1]|sfGetErrorColor}-->" size="5" />&nbsp;-
						<input class="inout_tx" type="text" name="<!--{$key2}-->" value="<!--{$arrForm[$key2].value|escape}-->" maxlength="<!--{$arrForm[$key2].length}-->" style="ime-mode: disabled; <!--{$arrErr[$key2]|sfGetErrorColor}-->" size="5" />&nbsp;-
						<input class="inout_tx" type="text" name="<!--{$key3}-->" value="<!--{$arrForm[$key3].value|escape}-->" maxlength="<!--{$arrForm[$key3].length}-->" style="ime-mode: disabled; <!--{$arrErr[$key3]|sfGetErrorColor}-->" size="5" />&nbsp;-
						<input class="inout_tx" type="text" name="<!--{$key4}-->" value="<!--{$arrForm[$key4].value|escape}-->" maxlength="<!--{$arrForm[$key4].length}-->" style="ime-mode: disabled; <!--{$arrErr[$key4]|sfGetErrorColor}-->" size="5" />
			...
			<div style="margin:10px 0 0 0">
				<table>
					<tr>
						<td class="lefttd">以上の内容で間違いなければ、下記「ご注文完了ページへ」ボタンをクリックしてください。<br />
           ...
           <!--{/if}-->
			<ul class="btn_area">
				<li class="prv_btn">
					<a href="javascript:void();" onclick="fnModeSubmit('return','',''); return false;">前のページへ戻る<input type="hidden" name="back03" id="back03" /></a>
				</li>
				<li class="nx_btn">
					<a href="javascript:void();" onclick="fnCheckSubmit('register', '', '');return false;">ご注文完了ページへ<input type="hidden" name="last" id="last" border="0" /></a>
				</li>
			</ul>
				<!--{if $enable_customer_regist}-->
					<table>
					...
					<ul class="btn_area">
						...
						<li class="nx_btn">
							<a href="javascript:void();" onclick="fnCheckSubmit('register', 'usecard', '1');return false;">ご注文完了ページへ<input type="hidden" name="last" id="last" /></a>
						</li>
					</ul>
			<!--{/if}-->
		</form>
