...
<dl>
    ...
    <!--{if $arrPayment[cnt].payment_reg == true}-->
    	<em class="payment_reg" style="display:block;"><!--{$arrPayment[cnt].payment_affairs_regular|escape|nl2br}--><!--{if $arrPayment[cnt].memo03 != '###'}--><!--{$arrPayment[cnt].payment_affairs|escape|nl2br}--><!--{/if}-->
        <!--{if $arrPayment[cnt].memo01 == 10 || $arrPayment[cnt].memo01 == 11}--><!--{$arrPayment[cnt].payment_affairs|escape|nl2br}--><!--{/if}-->
        </em>
    <!--{/if}-->
    <!--{if $arrPayment[cnt].payment_normal == true}-->
    	<em class="payment_normal" style="display:block;"><!--{$arrPayment[cnt].payment_affairs|escape|nl2br}--></em>
    <!--{/if}-->
    <!--{if $img_exist_flg}-->
...
<div class="bg_pay">
    ...
    <dd class="new_card_info">
        <ul class="smp-lp-username">
            <li style="width:80%;"><input type="text" name="card_no" value="<!--{$arrForm.card_no|escape}-->" class="w98" /></li>
        </ul>
    ...
    <dd class="new_card_info">
        <ul class="smp-lp-username">
            ...
            <li><input type="text" name="<!--{$key1}-->" value="<!--{$arrForm[$key1]|escape}-->" placeholder="TARO"  class="w95" /></li>
            <li><input type="text" name="<!--{$key2}-->" value="<!--{$arrForm[$key2]|escape}-->" placeholder="YAMADA"  class="w95" /></li>
        </ul>
        <span class="smp-lp-error"><!--{$arrErr[$key1]}--><!--{$arrErr[$key2]}--></span>
    </dd>
    </div>
    ...
    <dd>
        <ul>
          <li class="fl40"><input type="text" name="security_code" value="<!--{$arrForm.security_code|escape}-->"  class="w70"/>
          <!--{if $arrErr[$key]}--><span class="smp-lp-error"><!--{$arrErr[$key]}--></span><!--{/if}--></li>
          <li class="fl60"><img src="<!--{$TPL_DIR}-->img/shopping/card_01.png" width="194" height="57" alt="" align="middle" style="position:relative;top:-10px;left:-20px;" /></li>
       	</ul>
         <div class="clear" ></div>
    </dd>
    <!--{/if}-->
    ...
    <dd>
        ...
        <div class="card-registration_memo"><em>※すでに登録済みのカードが存在する場合、先に登録されたカード情報は削除されます。</em></div>
    </dd>
    </div>
    <!--{else}-->
...
<div class="bg_pay">
    ...
    <dd class="new_card_info">
        <ul class="smp-lp-username">
            <li><input type="tel" name="card_no" value="<!--{$arrForm.card_no|escape}-->" /></li>
        </ul>
        <span class="smp-lp-error"><!--{$arrErr.card_no}--></span>
    </dd>
...
<div class="bg_pay">
    ...
    <dd class="new_card_info">
        <ul class="smp-lp-username">
            <li style="width:80%;"><input type="tel" name="card_no" value="<!--{$arrForm.card_no|escape}-->" class="w98" id="gmo_card_no" /></li>
...
<!--{if $type == 7}-->
<dt><p class="txt_sz">▼お届け日を以下から選択してください</p></dt>  
...
<dd>
    <input type="radio" name="cycle_type" id="cycle_type3" class="cycle_type_radio" value="3" <!--{if $arrForm.cycle_type == 3}-->checked="checked"<!--{/if}--> /> <label for="cycle_type3" class="w100 h100">間隔で指定</label>
	<div id="cycle_type3_form" class="cycle_form"<!--{if $arrForm.cycle_type != 3}--> style="display:none;"<!--{/if}-->>
        <div id="cycle_interval_start_date_area" style="display:inline;">
            <div class="mgt10">初回お届け日
                <div class="custom-select2 w60">
...
<dd>	
       ...
       <!--お届け最短希望日-->
          ...
          <span class="ft_s">※希望しない場合はチェックを外して下さい</span>
        </div>
