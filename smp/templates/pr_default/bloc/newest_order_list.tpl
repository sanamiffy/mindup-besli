...
NewestOrders.prototype = {
    ...
    _getDefault:function() {
        ...
        params = {
            ...
            param:{mode:'default',max:100 , mobile:true},
    ...
    _setDefault:function(res) { 
        ...
        for (key in res.result) {
            var tr =  $('<tr class="orders">').append(
                                    ...
                                    $('<td>').html($('<a href="' + res.result[key]['site_url'] + 'smp/product/detail/' + res.result[key]['product_id'] + '">').html(res.result[key]['product_name'])).append(' ' + res.result[key]['create_date'])
                               );
    ...
    _update:function(){
        ...
        params = {
           url:'/api/?jb=api-newest_order_list',
           param:{mode:'update',max:100 , base_start_date:this._prevTime , base_end_date:this._objBaseDate.getTime() , mobile:true},
...
<tr class="index" id="order_list_header">
    <th width="70" class="newest_order_list_th" nowrap>写真</th>
    <th width="180" class="newest_order_list_th" nowrap>商品名</th>
</tr>
</table>
</div>
</body>
