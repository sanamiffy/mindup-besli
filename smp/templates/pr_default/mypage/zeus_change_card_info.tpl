...
$(document).ready(function() {
    ...
    if ($("#<!--{$action_use_new_card|escape:"javascript"}-->").length
    && $("#<!--{$action_use_saved_card|escape:"javascript"}-->").length
    ) {
        zeusInit();
    }else{
        mutation.observe(document.getElementById('zeus_token_card_info_area'), { childList: true, subtree: true });
    }
});
...
const zeusInit = () => {
    $('#zeus_token_action_type_new')[0].checked=true;
    $('#zeus_token_card_number')[0].placeholder="1234567890123456";
    $('#zeus_token_card_name')[0].placeholder="TARO YAMADA";
