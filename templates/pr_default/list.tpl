...
function lnSetSelect(name1, name2, id, val) {
        ...
        lists = eval('lists' + id);
        category_id1_stock_list = eval('category_id1_stock_list' + id);
        stock_lists = eval('stock_lists' + id);
        vals = eval('vals' + id);

		if (sele1) {
			category_id1_count = sele1.options.length;
			for(i = 0; i < category_id1_count; i++) {
				if (category_id1_stock_list[i] === '0') {
					sele1.options[i].disabled = true;
				}
			}
		}
				...
				<div class="in_cart">
					...
					<!--{if $arrProducts[cnt].regular_only_flg == 0}-->
						<div class="lst_cartbtn">
							<a onclick="fnChangeAction('<!--{request_uri|escape:javascript}-->#product<!--{$id|escape:javascript}-->'); fnModeSubmit('cart','product_id','<!--{$id|escape:javascript}-->'); return false;" class="js-prevent-form-submit">
					...
					<!--{if 'basis-lp_only'|get_option_use_flg == false}-->
						<!--{if $arrProducts[cnt].regular == 1}-->
							<div class="t_cartbtn2 mgt10">
								<a onclick="fnChangeAction('<!--{request_uri|escape:javascript}-->#product<!--{$id|escape:javascript}-->'); submitRegular('<!--{$id|escape:javascript}-->'); return false;" class="js-prevent-form-submit">
