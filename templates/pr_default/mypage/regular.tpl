   ...
   <div class="reglar_list">
       <table summary="申込履歴" class="regular_order_table">
          <tr>
            ...
            <th width="17%" class="centertd">申込番号<br>商品名</th>
            <th width="20%" class="centertd">お支払い方法</th>
            <th width="10%" class="centertd">合計金額</th>
            <th width="*" class="centertd">配送サイクル<br>次回お届け日</th>
          </tr>

      <!--{section name=cnt loop=$arrOrder}-->
            <tr <!--{if $arrOrder[cnt].is_pub == 0}-->class="clr_tr"<!--{/if}-->>
              <td rowspan="2"><!--{$arrOrder[cnt].create_date|sfDispDBDate}--></td>
              <td rowspan="2">
              <!--{$arrOrder[cnt].order_id}--><!--{if $arrOrder[cnt].is_pub == 0}--><span class="master_cancel">＜停止＞</span><!--{/if}--><br>
              <!--{if $arrOrder[cnt].regular_type == 1}-->
                  <!--{$arrOrder[cnt].name|mb_truncate:40|mb_strimwidth:0:100:'...':'UTF-8'|escape}--><br />(定期お申し込み)
              <!--{/if}-->
              </td>
              <td rowspan="2" class="centertd"><!--{$arrOrder[cnt].payment_method|escape}--></td>
              <td rowspan="2" class="centertd"><!--{$arrOrder[cnt].next_payment_total|number_format|escape}-->円</td>
              <td style="padding-top: 1rem;">次回お届け日：<!--{$arrOrder[cnt].regular_order_master.next_date|date_format:"%Y年%m月%d日"}--><br>サイクル：<!--{$arrOrder[cnt].regular_order_master.cycle_jp|escape}--></td>
            </tr>
            <tr class = "his_tr <!--{if $arrOrder[cnt].is_pub == 0}--> clr_tr<!--{/if}-->">
              <td class="centertd">
              <div class="list_area">
                <ul>
                ...
                <!--{/if}-->
                  <li>
                      <!--{if $arrOrder[cnt].is_pub != 0}-->
                        <a href="<!--{$smarty.server.PHP_SELF|escape}-->" onclick="fnChangeAction('./change_order_info.php'); fnKeySubmit('order_id','<!--{$arrOrder[cnt].order_id|escape:javascript}-->'); return false">
                                    ...
                                    <tr>
                                      ...
                                      <!--{section name=pr_cont loop=$arrOrder[cnt].products}-->
                                        <tr>
                                          ...
                                          <td class="centertd"><!--{$arrOrder[cnt].next_price[pr_cont]|number_format|escape}-->円</td>
                                          <td class="centertd">
                                            <span id="button_chenge_quantity<!--{$arrOrder[cnt].order_id|escape}-->"><!--{$arrOrder[cnt].products[pr_cont].quantity|number_format|escape}--></span>
                                            <!--{if $arrOrder[cnt].products[pr_cont].master_product && !($arrOrder[cnt].products[pr_cont].distribution)}-->
                                              <div style="margin-left:15px;">
                                                <input type="button" id="button_subscription_quantit_<!--{$arrOrder[cnt].order_id|escape}-->" style="width:100%" class="mg_dtbtn" onclick="changeQuantityView_toggle('<!--{$arrOrder[cnt].order_id|escape:javascript}-->');" value="数量を変更する"/>
                                              </div>
                                            <!--{/if}-->
                                          </td>
                                      ...
                                      <tr>
                                          ...
                                          <td class="pricetd" ><!--{$arrOrder[cnt].next_subtotal-$arrOrder[cnt].next_tax_total|number_format}-->円</td>
                                      </tr>
                                      <tr>
                                          <td colspan="3" class="righttd">消費税</td>
                                          <td class="pricetd"><!--{$arrOrder[cnt].next_tax_total|number_format}-->円</td>
                                      </tr>
                                      ...
                                      <tr>
                                          ...
                                          <td class="pricetd"><!--{$arrOrder[cnt].next_discount|number_format}-->円</td>
                                      </tr>
                                      <!--{/if}-->
                                      <tr>
                                          <td colspan="3" class="righttd">送料(税込)</td>
                                          <td class="pricetd"><!--{$arrOrder[cnt].next_deliv_fee|number_format}-->円</td>
                                      </tr>
                                      <!--{if $arrOrder[cnt].charge > 0}-->
                                      <tr>
                                          <td colspan="3" class="righttd">手数料(税込)</td>
                                          <td class="pricetd"><!--{$arrOrder[cnt].charge|number_format}-->円</td>
                                      </tr>
                                      <!--{/if}-->
                                      <!--{if $arrOrder[cnt].relay_fee > 0}-->
                                      <tr>
                                          <td colspan="3" class="righttd">離島手数料(税込)</td>
                                          <td class="pricetd"><!--{$arrOrder[cnt].relay_fee|number_format}-->円</td>
                                      </tr>
                                      <!--{/if}-->
                                      <tr class="totalbg">
                                          <td colspan="3" class="righttd">お支払い合計金額</td>
                                          <td class="pricetd"><span class="price"><!--{$arrOrder[cnt].next_payment_total|number_format}-->円</span></td>
                                      </tr>
                                 </tbody>
                            </table>
...
<!--{* 定期商品個数変更 *}-->
<div id="change_quantity_dialog" style="display:none" data-size="600">
    <form id="quantity_change">
        <table summary="定期商品個数変更">
            <tr>
                <td>ご希望の数量を選択してください</td>
                <td id="class_quantity_text">
                    <select name="class_quantity" id="class_quantity" style="width:50px;">
                    </select>
                </td>
            </tr>
        </table>
