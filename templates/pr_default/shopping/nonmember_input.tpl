...
<!--
	...
	<p>下記項目にご入力ください。<br />
	入力後、一番下の「同意して送信」ボタンをクリックしてください。</p>
	<form name="form1" id="form1" method="post" action="<!--{$smarty.server.PHP_SELF|escape}-->">
            ...
            <table summary="お客様情報入力">
				...
				<tr>
                    ...
                    <td>
						...
						<input type="text" name="<!--{$arrForm.$key.keyname}-->" value="<!--{$arrForm.$key.value|escape}-->" maxlength="<!--{$arrForm.$key.length}-->" style="<!--{$arrErr.$key|sfGetErrorColor}-->" size="18" class="nom_form" placeholder="09012345678"/>
					</td>
				</tr>
				<tr>
					<th>FAX</th>
                    ...
                    <td>
						<!--{assign var=key value="order_fax"}-->
						<span class="attention"><!--{$arrErr.$key}--></span>
						<input type="text" name="<!--{$arrForm.$key.keyname}-->" value="<!--{$arrForm.$key.value|escape}-->" maxlength="<!--{$arrForm.$key.length}-->" style="<!--{$arrErr.$key|sfGetErrorColor}-->" size="18" class="nom_form" placeholder="0312345678"/>
					</td>
				</tr>
				<tr>
					<th>メールアドレス</th>
                ...
                <div class="deliv_sitei"> 
					...
					<p class="txt_center fs18n">
						<input type="checkbox" id="non_member_agree" name="non_member_agree" value="1" />
						<a href="/privacy/index.php" target="_blank">プライバシーポリシー</a><span class="t_agree">に同意する</span>
					</p>
					<p class="txt_center" id="error_message_about_non_member_agree" style="display: none;"><em>※同意のチェックがされていません。</em></p>
                ...
                <table>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <span class="attention"><!--{$arrErr.$key}--></span>
                            <input type="text" name="<!--{$arrForm.$key.keyname}-->" value="<!--{$arrForm.$key.value|escape}-->" maxlength="<!--{$arrForm.$key.length}-->" style="<!--{$arrErr.$key|sfGetErrorColor}-->" size="18" placeholder="09012345678"/>
                        </td>
                    </tr>
				</table>
            </div>
		...
		<ul class="btn_area_l2">
			<li class="btn">
				<span class="btn_next">
					<a href="javascript:void(0);" onclick="changeErrorMessageDisplay();">同意して送信<input type="hidden"name="next" id="next" /></a>
				</span>
			</li>
		</ul>
...
<script>
	const agree_check   = document.getElementById('non_member_agree');
	const error_message = document.getElementById('error_message_about_non_member_agree');

	function changeErrorMessageDisplay () {
