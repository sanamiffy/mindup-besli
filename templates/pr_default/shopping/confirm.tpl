...
<div id="shoppingcolumn">
    ...
    <input type="hidden" name="<!--{$smarty.const.TRANSACTION_ID_NAME|escape}-->" value="<!--{$transactionid|escape}-->" />
		...
		<!--{/foreach}-->
			<tr>
				<td colspan="5" class="righttd">送料(税込)</td>
				<td class="pricetd"><!--{$arrData.deliv_fee|number_format}-->円</td>
			</tr>
			<tr>
				<td colspan="5" class="righttd">手数料(税込)</td>
			...
			<tr>
				<td colspan="5" class="righttd">離島手数料(税込)</td>
		...
		<div class="regular_dt">
			...
			<table summary="定期コース回数内訳">
				...
				<tr>
					<td class="cttd bd_rt bgnone" width="25%"><!--{$smarty.section.total_amount.iteration|escape}-->回目</td>
                    <td class="cttd bd_rt bgnone">
                        <!--{foreach from=$regular_product_list[$smarty.section.total_amount.iteration] key=key item=regular_product}-->
                            <!--{$regular_product.name}-->
			...
			<!--{/section}-->
                ...
                <tr>
                    ...
                    <!--{if $smarty.session.cycle_type == '3'}-->
                        <td colspan="3"><!--{$deliv_date_data.first_date_jp|escape}-->より<!--{$smarty.session.cycle_interval_interval}-->日間隔でお届け予定</td>
                    <!--{elseif $smarty.session.cycle_type == '1' || $smarty.session.cycle_type == '2'}-->
                        <td colspan="3"><!--{$deliv_date_data.first_date_jp|escape}-->より<!--{$date_text|escape}-->にお届け予定</td>
                    <!--{/if}-->
                </tr>
                <!--{if $total_amount_flg}-->
                <tr>
                ...
                *}-->
                ...
                <!--{* ここまで定期購入の場合 *}-->
                    <th>お届け回数</th>
                    <td colspan="3"><!--{$number_of_delivers|escape}--></td>
                </tr>
                <!--{/if}-->
			</table>
            <!--{if $total_amount_flg}-->
	...
	<!--{* ログイン済みの会員のみ *}-->
		...
		<table summary="お支払方法・お届け時間の指定・その他お問い合わせ" class="delivname">
            ...
            <tr>
                <th width="50%">お支払期限</th>
                <td><!--{$arrData.payment_deadline|escape}--></td>
            </tr>
            <!--{/if}-->
			<!--{if $reg_get != 1}-->
			<tr>
