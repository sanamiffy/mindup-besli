...
<!--{/if}-->
<!--{include file="`$smarty.const.TEMPLATE_DIR`init_timer_bloc.tpl"}-->
<!--{$GLOBAL_ERR}-->
<noscript>
...
<div id="wrapper">
	...
	<!--{* ▼HEADER *}-->
	<!--{if $smarty.server.REQUEST_URI|replace:"index.php":"" != '/forgot/'}-->
	...
	<!--{/if}-->
		...
		<!--{* ▼CENTER COLUMN *}-->
	<!--{if $smarty.server.REQUEST_URI|replace:"index.php":"" != '/forgot/'}-->
		<!--{if $tpl_mainpage == 'shopping/input_day.tpl'}-->
			<!--{assign var=tpl_column_num value="1"}-->
		<!--{/if}-->
		<!--{if $tpl_column_num == 3}-->
			<div id="three_maincolumn">
		<!--{elseif $tpl_mainpage|strpos:'shopping/paygent_'===0}-->
