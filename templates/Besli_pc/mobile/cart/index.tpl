...
<form name="form1" id="form1" method="post" action="<!--{$smarty.server.PHP_SELF|escape}-->">
	...
	<input type="hidden" name="transactionid" value="<!--{$smarty.session.transactionid}-->">
	...
	<!--{section name=cnt loop=$arrProductsClass}-->
		...
		<a href="<!--{$smarty.server.PHP_SELF|escape}-->?mode=up&amp;cart_no=<!--{$arrProductsClass[cnt].cart_no}-->&transactionid=<!--{$smarty.session.transactionid}-->">+</a>
		<a href="<!--{$smarty.server.PHP_SELF|escape}-->?mode=down&amp;cart_no=<!--{$arrProductsClass[cnt].cart_no}-->&transactionid=<!--{$smarty.session.transactionid}-->">-</a>
		<a href="<!--{$smarty.server.PHP_SELF|escape}-->?mode=delete&amp;cart_no=<!--{$arrProductsClass[cnt].cart_no}-->&transactionid=<!--{$smarty.session.transactionid}-->">削除</a><br>
		<!--{* 合計 *}-->
...
<a href="<!--{$smarty.const.MOBILE_URL_SITE_TOP}-->?transactionid=<!--{$smarty.session.transactionid}-->&PHPSESSID=<!--{$smarty.request.PHPSESSID}-->" accesskey="0"><!--{0|numeric_emoji}-->TOPページへ</a><br>
