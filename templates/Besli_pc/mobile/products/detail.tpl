...
<form name="form1" method="post" action="<!--{$smarty.server.PHP_SELF|escape}-->">
	...
	<input type="hidden" name="transactionid" value="<!--{$smarty.post.transactionid|escape}-->">
	<input type="hidden" name="product_id" value="<!--{$tpl_product_id|escape}-->">
<!--{if $tpl_stock_find}-->
	<!--★商品を選ぶ★-->
...
<a href="<!--{$smarty.const.MOBILE_URL_CART_TOP}-->?transactionid=<!--{$smarty.get.transactionid}-->&PHPSESSID=<!--{php}-->echo session_id();<!--{/php}-->" accesskey="9"><!--{9|numeric_emoji}-->かごを見る</a><br>
<a href="<!--{$smarty.const.MOBILE_URL_SITE_TOP}-->?transactionid=<!--{$smarty.get.transactionid}-->&PHPSESSID=<!--{php}-->echo session_id();<!--{/php}-->" accesskey="0"><!--{0|numeric_emoji}-->TOPページへ</a><br>
