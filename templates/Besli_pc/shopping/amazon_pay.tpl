            ...
            <tr>
                ...
                <td class="centertd">
                    <!--{if $arrProductsClass[cnt].price02 != "" || strlen($arrProductsClass[cnt].price02)}-->
                        <!--{$arrProductsClass[cnt].price02|sfPreTax:$arrInfo.tax:$arrInfo.tax_rule|number_format|escape}-->円
                    <!--{else}-->
                        <!--{$arrProductsClass[cnt].price01|sfPreTax:$arrInfo.tax:$arrInfo.tax_rule|number_format|escape}-->円
                    <!--{/if}-->
                </td>
...
<!--{assign var=customer_pref value=$customer|@null_coalescing:"pref"}-->
        ...
        <table summary="お届け時間の指定・その他お問い合わせ" class="delivname wait_amazon_area" style="display:none;">
            <tbody>
                ...
                <tr><!--{* 初回即時、もしくは初回最短機能を使いたいが、チェックボックスを表示したくない場合、左の <div class="btn_area">を<div class="btn_area" style="display:none;">に変更してください *}-->
                    <td class="no_bdr no_pd">
                        <!--{*
                        === 最初からチェックを入れた状態にしたい場合 ===
        ...
        <div class="tblareabtn">
            ...
            <div class="btn_rpst2" id="btn_next_area">
                <span class="btn_next" id="btn_next" onclick="AmazonPaySubmit('form1');" style="display:none;">
                    <a href="javascript:void(0);">ご注文完了ページへ<input type="hidden" name="next" id="next" /></a>
                </span>
            </div>
        </div>
    </form>
