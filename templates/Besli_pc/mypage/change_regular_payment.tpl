       ...
       <table>    
		...
		<tr>
        	<td colspan="5">
            	<!--{if $arrPayment}-->
                     <div class="payment_info">
                        <h3>▼お支払い方法をお選び下さい。</h3>
                            ...
                            <ul>
                                <li>
                                    <input type="radio" name="payment_id" id="payment_<!--{$item.payment_id}-->" value="<!--{$item.payment_id}-->"<!--{if $item.regular_credit_flg}--> class="credit_check<!--{if $item.gmo_flg}--> gmo_credit<!--{/if}-->"<!--{/if}--> <!--{if $item.payment_id == $arrForm.payment_id.value}-->checked="checked"<!--{/if}--> />
             ...
             <!--{/if}-->
                <div class="mg_newbtn">
                        <a href="javascript:void();" onclick="paygent_token_send();">支払方法を変更</a>
                </div>
            <!--{else}-->
            ...
            <!--{/if}-->
               <p style="display:none;">
                        <span class="btn_prev"><a href="#" onClick="history.back(); return false;">前のページへ</a></span>
                </div>
...
<!--{/if}-->
  <em>※「配送回数が0回」または「初回最短お届け日」が指定されている場合、お支払い方法を変更できません。</em>
	<div class="btn_rpst3_2">
            <span class="btn_mgprev"><a href="#" onClick="history.back(); return false;">前のページへ</a></span>
    </div>
...
$(function(){
	...
	$('input[name=payment_id]').click(function() {
		if ($(this).hasClass('credit_check')) {
			...
			if ($(this).hasClass('gmo_credit')) {
				...
				<!--{if $arrSavedCardInfo.regular}-->$('.credit_info').hide();<!--{/if}-->
				$('.gmo_method').hide();
				$('.credit_paygent').show();
			}
		} else {
	...
	if ($('input[name=payment_id]:checked').hasClass('credit_check')) {
		$('input[name=credit]').val('1');
		$('.credit_info').show();
		if ($('input[name=payment_id]:checked').hasClass('gmo_credit')) {
			$('.gmo_method').show();
			$('.credit_paygent').hide();
		} else {
			<!--{if $arrSavedCardInfo.regular}-->$('.credit_info').hide();<!--{/if}-->
